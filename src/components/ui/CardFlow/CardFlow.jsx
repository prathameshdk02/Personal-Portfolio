...
const CardFlow = ({ cards, offset, innerWidth }) => {
  ...
  const cardFlowCardMap = cards.map((card, index) => (
    <CardFlowCard
      ...
      offsetIndex={index - currentIndex}></CardFlowCard>
  ));
    ...
    <div className="flex items-center relative -mx-2">
      <button
        ...
        <FontAwesomeIcon
          ...
          } pointer-events-none`}></FontAwesomeIcon>
      </button>

      <div className="flex-1 overflow-hidden">
        <div className="relative min-h-[700px] md:min-h-[600px] z-0">{cardFlowCardMap}</div>
      </div>

      <button
        onClick={(event) => {
          event.stopPropagation();
          setCurrentIndex((prev) => (prev == cards.length - 1 ? cards.length - 1 : prev + 1));
        }}
        onDoubleClick={(event) => {
          event.stopPropagation();
        }}
